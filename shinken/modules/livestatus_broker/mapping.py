...
livestatus_attribute_map = {
    'Host': {
        'accept_passive_checks': {
            'description': intern('Whether passive host checks are accepted (0/1)'),
            'function': lambda item, req: item.passive_checks_enabled,
            'datatype': bool,
        },
        'acknowledged': {
            'description': intern('Whether the current host problem has been acknowledged (0/1)'),
            'function': lambda item, req: item.problem_has_been_acknowledged,
            'datatype': bool,
        },
        'acknowledgement_type': {
            'description': intern('Type of acknowledgement (0: none, 1: normal, 2: stick)'),
            'function': lambda item, req: item.acknowledgement_type,
            'datatype': int,
        },
        'action_url': {
            'description': intern('An optional URL to custom actions or information about this host'),
            'function': lambda item, req: item.action_url,
        },
        'action_url_expanded': {
            'description': intern('The same as action_url, but with the most important macros expanded'),
            'function': lambda item, req: MacroResolver().resolve_simple_macros_in_string(item.action_url, item.get_data_for_checks()),
        },
        'active_checks_enabled': {
            'description': intern('Whether active checks are enabled for the host (0/1)'),
            'function': lambda item, req: item.active_checks_enabled,
            'datatype': bool,
        },
        'address': {
            'description': intern('IP address'),
            'function': lambda item, req: item.address,
        },
        'alias': {
            'description': intern('An alias name for the host'),
            'function': lambda item, req: item.alias,
        },
        'business_impact': {
            'description': intern('The importance we gave to this host between hte minimum 0 and the maximum 5'),
            'function': lambda item, req: item.business_impact,
            'datatype': int,
        },
        'check_command': {
            'description': intern('Nagios command for active host check of this host'),
            'function': lambda item, req: item.check_command.get_name(),
        },
        'check_flapping_recovery_notification': {
            'description': intern('Whether to check to send a recovery notification when flapping stops (0/1)'),
            'function': lambda item, req: item.check_flapping_recovery_notification,  # REPAIRME WTF
            'datatype': int,
        },
        'check_freshness': {
            'description': intern('Whether freshness checks are activated (0/1)'),
            'function': lambda item, req: item.check_freshness,
            'datatype': bool,
        },
        'check_interval': {
            'description': intern('Number of basic interval lengths between two scheduled checks of the host'),
            'function': lambda item, req: item.check_interval,
            'datatype': float,
        },
        'check_options': {
            'description': intern('The current check option, forced, normal, freshness... (0-2)'),
            'function': lambda item, req: "",  # REPAIRME
            'datatype': int,
        },
        'check_period': {
            'description': intern('Time period in which this host will be checked. If empty then the host will always be checked.'),
            'function': lambda item, req: item.check_period.get_name(),
        },
        'check_type': {
            'description': intern('Type of check (0: active, 1: passive)'),
            'function': lambda item, req: item.check_type,
            'datatype': int,
        },
        'checks_enabled': {
            'description': intern('Whether checks of the host are enabled (0/1)'),
            'function': lambda item, req: item.active_checks_enabled,
            'datatype': bool,
        },
        'child_dependencies': {
            'description': intern('List of the host/service that depend on this host (logical, network or business one).'),
            'function': lambda item, req: from_svc_hst_distinct_lists(item.child_dependencies),
            'datatype': list,
        },
        'childs': {
            'description': intern('A list of all direct childs of the host'),
            'function': lambda item, req: [x.get_name() for x in item.childs],
            'datatype': list,
        },
        'comments': {
            'description': intern('A list of the ids of all comments of this host'),
            'function': lambda item, req: [x.id for x in item.comments],
            'datatype': list,
        },
        'comments_with_info': {
            'description': intern('A list of the ids of all comments of this host with id, author and comment'),
            'function': lambda item, req: [join_with_separators(req, str(x.id), x.author, x.comment) for x in item.comments],  # REPAIRME MAYBE
            'datatype': list,
        },
        'contacts': {
            'description': intern('A list of all contacts of this host, either direct or via a contact group'),
            'function': lambda item, req: [x.contact_name for x in item.contacts],
            'datatype': list,
        },
        'contact_groups': {
            'description': intern('A list of all contact groups this host is in'),
        ...
        'current_attempt': {
            'description': intern('Number of the current check attempts'),
            'function': lambda item, req: item.attempt,
            'datatype': int,
        },
        'current_notification_number': {
            'description': intern('Number of the current notification'),
            'function': lambda item, req: item.current_notification_number,
            'datatype': int,
        },
        'custom_variable_names': {
            'description': intern('A list of the names of all custom variables'),
            'function': lambda item, req: get_customs_keys(item.customs),
            'datatype': list,
        },
        'custom_variable_values': {
            'description': intern('A list of the values of the custom variables'),
            'function': lambda item, req: get_customs_values(item.customs),
            'datatype': list,
        },
        'custom_variables': {
            'description': intern('A dictionary of the custom variables'),
            'function': lambda item, req: [join_with_separators(req, k[1:], v) for k, v in item.customs.iteritems()],
            'datatype': list,
        },
        'display_name': {
            'description': intern('Optional display name of the host - not used by Nagios\' web interface'),
            'function': lambda item, req: item.host_name,
        },
        'downtimes': {
            'description': intern('A list of the ids of all scheduled downtimes of this host'),
            'function': lambda item, req: [x.id for x in item.downtimes],
            'datatype': list,
        },
        'downtimes_with_info': {
            'description': intern('A list of the all scheduled downtimes of the host with id, author and comment'),
        ...
        'event_handler': {
            'description': intern('Nagios command used as event handler'),
            'function': lambda item, req: item.event_handler.get_name(),
        },
        'event_handler_enabled': {
            'description': intern('Whether event handling is enabled (0/1)'),
            'function': lambda item, req: item.event_handler_enabled,
            'datatype': bool,
        },
        'execution_time': {
            'description': intern('Time the host check needed for execution'),
            'function': lambda item, req: item.execution_time,
            'datatype': float,
        },
        'filename': {
            'description': intern('The value of the custom variable FILENAME'),
            'function': lambda item, req: '',  # REPAIRME
            'datatype': str,
        },
        'first_notification_delay': {
            'description': intern('Delay before the first notification'),
            'function': lambda item, req: item.first_notification_delay,
            'datatype': float,
        },
        'flap_detection_enabled': {
            'description': intern('Whether flap detection is enabled (0/1)'),
            'function': lambda item, req: item.flap_detection_enabled,
            'datatype': bool,
        },
        'got_business_rule': {
            'description': intern('Whether the host state is an business rule based host or not (0/1)'),
            'function': lambda item, req: item.got_business_rule,
            'datatype': bool,
        },
        'groups': {
            'description': intern('A list of all host groups this host is in'),
            'function': lambda item, req: [x.get_name() for x in item.hostgroups],
            'datatype': list,
        },
        'hard_state': {
            'description': intern('The effective hard state of the host (eliminates a problem in hard_state)'),
            'function': lambda item, req: "",  # REPAIRME
            'datatype': int,
        },
        'has_been_checked': {
            'description': intern('Whether the host has already been checked (0/1)'),
            'function': lambda item, req: item.has_been_checked,
            'datatype': int,
        },
        'high_flap_threshold': {
            'description': intern('High threshold of flap detection'),
            'function': lambda item, req: item.high_flap_threshold,
            'datatype': float,
        },
        'icon_image': {
            'description': intern('The name of an image file to be used in the web pages'),
            'function': lambda item, req: item.icon_image,
        },
        'icon_image_alt': {
            'description': intern('Alternative text for the icon_image'),
            'function': lambda item, req: item.icon_image_alt,
        },
        'icon_image_expanded': {
            'description': intern('The same as icon_image, but with the most important macros expanded'),
            'function': lambda item, req: MacroResolver().resolve_simple_macros_in_string(item.icon_image, item.get_data_for_checks()),
        },
        'impacts': {
            'description': intern('List of what the source impact (list of hosts and services)'),
        ...
        'in_check_period': {
            'description': intern('Whether this host is currently in its check period (0/1)'),
            'function': lambda item, req: (item.check_period is None and [False] or [item.check_period.is_time_valid(req.tic)])[0],
            'datatype': bool,
        },
        'in_notification_period': {
            'description': intern('Whether this host is currently in its notification period (0/1)'),
            'function': lambda item, req: (item.notification_period is None and [False] or [item.notification_period.is_time_valid(req.tic)])[0],
            'datatype': bool,
        },
        'initial_state': {
            'description': intern('Initial host state'),
            'function': lambda item, req: item.initial_state,
            'datatype': int,
        },
        'is_executing': {
            'description': intern('is there a host check currently running... (0/1)'),
            'function': lambda item, req: False,  # REPAIRME # value in scheduler is not real-time
            'datatype': bool,
        },
        'is_flapping': {
            'description': intern('Whether the host state is flapping (0/1)'),
            'function': lambda item, req: item.is_flapping,
            'datatype': bool,
        },
        'is_impact': {
            'description': intern('Whether the host state is an impact or not (0/1)'),
            'function': lambda item, req: item.is_impact,
            'datatype': bool,
        },
        'is_problem': {
            'description': intern('Whether the host state is a problem or not (0/1)'),
            'function': lambda item, req: item.is_problem,
            'datatype': bool,
        },
        'last_check': {
            'description': intern('Time of the last check (Unix timestamp)'),
            'function': lambda item, req: int(item.last_chk),
            'datatype': int,
        },
        'last_hard_state': {
            'description': intern('Last hard state'),
            'function': lambda item, req: item.last_hard_state,
            'datatype': int,
        },
        'last_hard_state_change': {
            'description': intern('Time of the last hard state change (Unix timestamp)'),
            'function': lambda item, req: item.last_hard_state_change,
            'datatype': int,
        },
        'last_notification': {
            'description': intern('Time of the last notification (Unix timestamp)'),
            'function': lambda item, req: int(item.last_notification),
            'datatype': int,
        },
        'last_state': {
            'description': intern('State before last state change'),
            'function': lambda item, req: item.last_state,
            'datatype': int,
        },
        'last_state_change': {
            'description': intern('Time of the last state change - soft or hard (Unix timestamp)'),
            'function': lambda item, req: int(item.last_state_change),
            'datatype': int,
        },
        'last_time_down': {
            'description': intern('The last time the host was DOWN (Unix timestamp)'),
            'function': lambda item, req: item.last_time_down,  # REPAIRME
            'datatype': int,
        },
        'last_time_unreachable': {
            'description': intern('The last time the host was UNREACHABLE (Unix timestamp)'),
            'function': lambda item, req: item.last_time_unreachable,  # REPAIRME
            'datatype': int,
        },
        'last_time_up': {
            'description': intern('The last time the host was UP (Unix timestamp)'),
            'function': lambda item, req: item.last_time_up,  # REPAIRME
            'datatype': int,
        },
        'latency': {
            'description': intern('Time difference between scheduled check time and actual check time'),
            'function': lambda item, req: item.latency,
            'datatype': float,
        },
        'long_plugin_output': {
            'description': intern('Complete output from check plugin'),
            'function': lambda item, req: item.long_output,
        },
        'low_flap_threshold': {
            'description': intern('Low threshold of flap detection'),
            'function': lambda item, req: item.low_flap_threshold,
            'datatype': float,
        },
        'max_check_attempts': {
            'description': intern('Max check attempts for active host checks'),
            'function': lambda item, req: item.max_check_attempts,
            'datatype': int,
        },
        'modified_attributes': {
            'description': intern('A bitmask specifying which attributes have been modified'),
            'function': lambda item, req: item.modified_attributes,  # CONTROLME 
            'datatype': int,
        },
        'modified_attributes_list': {
            'description': intern('A list of all modified attributes'),
            'function': lambda item, req: modified_attributes_names(item),  # CONTROLME
            'datatype': list,
        },
        'name': {
            'description': intern('Host name'),
            'function': lambda item, req: item.host_name,
        },
        'next_check': {
            'description': intern('Scheduled time for the next check (Unix timestamp)'),
            'function': lambda item, req: int(item.next_chk),
            'datatype': int,
        },
        'next_notification': {
            'description': intern('Time of the next notification (Unix timestamp)'),
            'function': lambda item, req: int(item.last_notification + item.notification_interval * item.__class__.interval_length),  # CONTROLME
            'datatype': int,
        },
        'notes': {
            'description': intern('Optional notes for this host'),
            'function': lambda item, req: item.notes,
        },
        'notes_expanded': {
            'description': intern('The same as notes, but with the most important macros expanded'),
            'function': lambda item, req: MacroResolver().resolve_simple_macros_in_string(item.notes, item.get_data_for_checks()),
        },
        'notes_url': {
            'description': intern('An optional URL with further information about the host'),
            'function': lambda item, req: item.notes_url,
        },
        'notes_url_expanded': {
            'description': intern('Same es notes_url, but with the most important macros expanded'),
            'function': lambda item, req: MacroResolver().resolve_simple_macros_in_string(item.notes_url, item.get_data_for_checks()),
        },
        'notification_interval': {
            'description': intern('Interval of periodic notification or 0 if its off'),
            'function': lambda item, req: item.notification_interval,
            'datatype': float,
        },
        'notification_period': {
            'description': intern('Time period in which problems of this host will be notified. If empty then notification will be always'),
            'function': lambda item, req: item.notification_period.get_name(),
        },
        'notifications_enabled': {
            'description': intern('Whether notifications of the host are enabled (0/1)'),
            'function': lambda item, req: item.notifications_enabled,
            'datatype': bool,
        },
        'no_more_notifications': {
            'description': intern('Whether to stop sending notifications (0/1)'),
            'function': lambda item, req: item.no_more_notifications,  # REPAIRME, maybe ask both instance and class
            'datatype': bool,
        },
        'num_services': {
            'description': intern('The total number of services of the host'),
            'function': lambda item, req: len(item.services),
        },
        'num_services_crit': {
            'description': intern('The number of the host\'s services with the soft state CRIT'),
            'function': lambda item, req: len([x for x in item.services if x.state_id == 2]),
        },
        'num_services_hard_crit': {
            'description': intern('The number of the host\'s services with the hard state CRIT'),
            'function': lambda item, req: len([x for x in item.services if x.state_id == 2 and x.state_type_id == 1]),
        },
        'num_services_hard_ok': {
            'description': intern('The number of the host\'s services with the hard state OK'),
            'function': lambda item, req: len([x for x in item.services if x.state_id == 0 and x.state_type_id == 1]),
        },
        'num_services_hard_unknown': {
            'description': intern('The number of the host\'s services with the hard state UNKNOWN'),
            'function': lambda item, req: len([x for x in item.services if x.state_id == 3 and x.state_type_id == 1]),
        },
        'num_services_hard_warn': {
            'description': intern('The number of the host\'s services with the hard state WARN'),
            'function': lambda item, req: len([x for x in item.services if x.state_id == 1 and x.state_type_id == 1]),
        },
        'num_services_ok': {
            'description': intern('The number of the host\'s services with the soft state OK'),
            'function': lambda item, req: len([x for x in item.services if x.state_id == 0]),
        },
        'num_services_pending': {
            'description': intern('The number of the host\'s services which have not been checked yet (pending)'),
            'function': lambda item, req: len([x for x in item.services if x.has_been_checked == 0]),
        },
        'num_services_unknown': {
            'description': intern('The number of the host\'s services with the soft state UNKNOWN'),
            'function': lambda item, req: len([x for x in item.services if x.state_id == 3]),
        },
        'num_services_warn': {
            'description': intern('The number of the host\'s services with the soft state WARN'),
            'function': lambda item, req: len([x for x in item.services if x.state_id == 1]),
        },
        'obsess_over_host': {
            'description': intern('The current obsess_over_host setting... (0/1)'),
            'function': lambda item, req: item.obsess_over_host,
            'datatype': bool,
        },
        'parent_dependencies': {
            'description': intern('List of the dependencies (logical, network or business one) of this host.'),
            'function': lambda item, req: from_svc_hst_distinct_lists(item.parent_dependencies),
            'datatype': list,
        },
        'parents': {
            'description': intern('A list of all direct parents of the host'),
            'function': lambda item, req: [x.get_name() for x in item.parents],
            'datatype': list,
        },
        'pending_flex_downtime': {
            'description': intern('Whether a flex downtime is pending (0/1)'),
            'function': lambda item, req: item.pending_flex_downtime,
            'datatype': int,
        },
        'percent_state_change': {
            'description': intern('Percent state change'),
            'function': lambda item, req: item.percent_state_change,
            'datatype': float,
        },
        'perf_data': {
            'description': intern('Optional performance data of the last host check'),
            'function': lambda item, req: item.perf_data,
        },
        'plugin_output': {
            'description': intern('Output of the last host check'),
            'function': lambda item, req: item.output,
        },
        'pnpgraph_present': {
            'description': intern('Whether there is a PNP4Nagios graph present for this host (0/1)'),
            'function': lambda item, req: find_pnp_perfdata_xml(item.get_name(), req),
            'datatype': int,
        },
        'process_performance_data': {
            'description': intern('Whether processing of performance data is enabled (0/1)'),
            'function': lambda item, req: item.process_perf_data,
            'datatype': bool,
        },
        'retry_interval': {
            'description': intern('Number of basic interval lengths between checks when retrying after a soft error'),
            'function': lambda item, req: item.retry_interval,
            'datatype': float,
        },
        'scheduled_downtime_depth': {
            'description': intern('The number of downtimes this host is currently in'),
            'function': lambda item, req: item.scheduled_downtime_depth,
            'datatype': int,
        },
        'services': {
            'description': intern('A list of all services of the host'),
            'function': lambda item, req: [x.get_name() for x in item.services],
            'datatype': list,
        },
        'services_with_info': {
            'description': intern('A list of all services including detailed information about each service'),
        ...
        'services_with_state': {
            'description': intern('A list of all services of the host together with state and has_been_checked'),
            'function': lambda item, req: [join_with_separators(req, x.get_name(), x.state_id, x.has_been_checked) for x in item.services],
            'datatype': list,
        },
        'source_problems': {
            'description': intern('The name of the source problems (host or service)'),
        ...
        'state': {
            'description': intern('The current state of the host (0: up, 1: down, 2: unreachable)'),
            'function': lambda item, req: item.state_id,
            #'function': i_am_state,
            'datatype': int,
        },
        'state_type': {
            'description': intern('Type of the current state (0: soft, 1: hard)'),
            'function': lambda item, req: item.state_type_id,
            'datatype': int,
        },
        'statusmap_image': {
            'description': intern('The name of in image file for the status map'),
        ...
        'worst_service_hard_state': {
            'description': intern('The worst hard state of all of the host\'s services (OK <= WARN <= UNKNOWN <= CRIT)'),
            'function': lambda item, req: reduce(worst_service_state, (x.state_id for x in item.services if x.state_type_id == 1), 0),
            'datatype': int,
        },
        'worst_service_state': {
            'description': intern('The worst soft state of all of the host\'s services (OK <= WARN <= UNKNOWN <= CRIT)'),
            'function': lambda item, req: reduce(worst_service_state, (x.state_id for x in item.services), 0),
            'datatype': int,
        },
        'x_3d': {
            'description': intern('3D-Coordinates: X'),
            'function': lambda item, req: "",  # REPAIRME
            'datatype': float,
        },
        'y_3d': {
            'description': intern('3D-Coordinates: Y'),
            'function': lambda item, req: "",  # REPAIRME
            'datatype': float,
        },
        'z_3d': {
            'description': intern('3D-Coordinates: Z'),
            'function': lambda item, req: "",  # REPAIRME
            'datatype': float,
        },
    },
    'Service': {
        'accept_passive_checks': {
            'description': intern('Whether the service accepts passive checks (0/1)'),
            'function': lambda item, req: item.passive_checks_enabled,
            'datatype': bool,
        },
        'acknowledged': {
            'description': intern('Whether the current service problem has been acknowledged (0/1)'),
            'function': lambda item, req: item.problem_has_been_acknowledged,
            'datatype': bool,
        },
        'acknowledgement_type': {
            'description': intern('The type of the acknownledgement (0: none, 1: normal, 2: sticky)'),
            'function': lambda item, req: item.acknowledgement_type,
            'datatype': int,
        },
        'action_url': {
            'description': intern('An optional URL for actions or custom information about the service'),
            'function': lambda item, req: item.action_url,
        },
        'action_url_expanded': {
            'description': intern('The action_url with (the most important) macros expanded'),
            'function': lambda item, req: MacroResolver().resolve_simple_macros_in_string(item.action_url, item.get_data_for_checks()),
        },
        'active_checks_enabled': {
            'description': intern('Whether active checks are enabled for the service (0/1)'),
            'function': lambda item, req: item.active_checks_enabled,
            'datatype': bool,
        },
        'business_impact': {
            'description': intern('The importance we gave to this service between hte minimum 0 and the maximum 5'),
            'function': lambda item, req: item.business_impact,
            'datatype': int,
        },
        'check_command': {
            'description': intern('Nagios command used for active checks'),
            'function': lambda item, req: item.check_command.get_name(),
        },
        'check_interval': {
            'description': intern('Number of basic interval lengths between two scheduled checks of the service'),
            'function': lambda item, req: item.check_interval,
            'datatype': float,
        },
        'check_options': {
            'description': intern('The current check option, forced, normal, freshness... (0/1)'),
            'function': lambda item, req: "",  # REPAIRME
            'datatype': int,
        },
        'check_period': {
            'description': intern('The name of the check period of the service. It this is empty, the service is always checked.'),
            'function': lambda item, req: item.check_period.get_name(),
        },
        'check_type': {
            'description': intern('The type of the last check (0: active, 1: passive)'),
        ...
        'child_dependencies': {
            'description': intern('List of the host/service that depend on this service (logical, network or business one).'),
            'function': lambda item, req: from_svc_hst_distinct_lists(item.child_dependencies),
            'datatype': list,
        },
        'comments': {
            'description': intern('A list of all comment ids of the service'),
            'function': lambda item, req: [x.id for x in item.comments],
            'datatype': list,
        },
        'comments_with_info': {
            'description': intern('A list of the ids of all comments of this service with id, author and comment'),
            'function': lambda item, req: [join_with_separators(req, str(x.id), x.author, x.comment) for x in item.comments],  # REPAIRME MAYBE
            'datatype': list,
        },
        'contacts': {
            'description': intern('A list of all contacts of the service, either direct or via a contact group'),
        ...
        'contact_groups': {
            'description': intern('A list of all contact groups this service is in'),
            'function': lambda item, req: item.contact_groups.split(','),
            'datatype': list,
        },
        'criticity': {
        ...
        'current_attempt': {
            'description': intern('The number of the current check attempt'),
            'function': lambda item, req: item.attempt,
            'datatype': int,
        },
        'current_notification_number': {
            'description': intern('The number of the current notification'),
            'function': lambda item, req: item.current_notification_number,
            'datatype': int,
        },
        'custom_variables': {
            'description': intern('A dictorionary of the custom variables'),
            'function': lambda item, req: [join_with_separators(req, k[1:], v) for k, v in item.customs.iteritems()],
            'datatype': list,
        },
        'custom_variable_names': {
            'description': intern('A list of the names of all custom variables of the service'),
            'function': lambda item, req: get_customs_keys(item.customs),
            'datatype': list,
        },
        'custom_variable_values': {
            'description': intern('A list of the values of all custom variable of the service'),
            'function': lambda item, req: get_customs_values(item.customs),
            'datatype': list,
        },
        'description': {
            'description': intern('Description of the service (also used as key)'),
            'function': lambda item, req: item.service_description,
        },
        'display_name': {
            'description': intern('An optional display name (not used by Nagios standard web pages)'),
            'function': lambda item, req: item.service_description,
        },
        'downtimes': {
            'description': intern('A list of all downtime ids of the service'),
            'function': lambda item, req: [x.id for x in item.downtimes],
            'datatype': list,
        },
        'downtimes_with_info': {
            'description': intern('A list of all downtimes of the service with id, author and comment'),
            'function': lambda item, req: [join_with_separators(req, str(x.id), x.author, x.comment) for x in item.downtimes],  # REPAIRME MAYBE
            'datatype': list,
        },
        'event_handler': {
        ...
        'event_handler_enabled': {
            'description': intern('Whether and event handler is activated for the service (0/1)'),
            'function': lambda item, req: item.event_handler_enabled,
            'datatype': bool,
        },
        'execution_time': {
        ...
        'flap_detection_enabled': {
            'description': intern('Whether flap detection is enabled for the service (0/1)'),
            'function': lambda item, req: item.flap_detection_enabled,
            'datatype': bool,
        },
        'got_business_rule': {
            'description': intern('Whether the service state is an business rule based host or not (0/1)'),
            'function': lambda item, req: item.got_business_rule,
            'datatype': bool,
        },
        'groups': {
            'description': intern('A list of all service groups the service is in'),
            'function': lambda item, req: [x.get_name() for x in item.servicegroups],
            'datatype': list,
        },
        'has_been_checked': {
            'description': intern('Whether the service already has been checked (0/1)'),
            'function': lambda item, req: item.has_been_checked,
            'datatype': int,
        },
        'high_flap_threshold': {
        ...
        'icon_image': {
            'description': intern('The name of an image to be used as icon in the web interface'),
            'function': lambda item, req: item.icon_image,
        },
        'icon_image_alt': {
            'description': intern('An alternative text for the icon_image for browsers not displaying icons'),
            'function': lambda item, req: item.icon_image_alt,
        },
        'icon_image_expanded': {
            'description': intern('The icon_image with (the most important) macros expanded'),
            'function': lambda item, req: MacroResolver().resolve_simple_macros_in_string(item.icon_image, item.get_data_for_checks()),
        },
        'impacts': {
            'description': intern('List of what the source impact (list of hosts and services)'),
        ...
        'in_check_period': {
            'description': intern('Whether the service is currently in its check period (0/1)'),
            'function': lambda item, req: (item.check_period is None and [False] or [item.check_period.is_time_valid(req.tic)])[0],
            'datatype': int,
        },
        'in_notification_period': {
            'description': intern('Whether the service is currently in its notification period (0/1)'),
            'function': lambda item, req: (item.notification_period is None and [False] or [item.notification_period.is_time_valid(req.tic)])[0],
            'datatype': int,
        },
        'initial_state': {
            'description': intern('The initial state of the service'),
            'function': lambda item, req: item.initial_state,
            'datatype': int,
        },
        'is_executing': {
            'description': intern('is there a service check currently running... (0/1)'),
            'function': lambda item, req: 0,  # REPAIRME # value in scheduler is not real-time
            'datatype': bool,
        },
        'is_flapping': {
            'description': intern('Whether the service is flapping (0/1)'),
            'function': lambda item, req: item.is_flapping,
            'datatype': bool,
        },
        'is_impact': {
        ...
        'last_check': {
            'description': intern('The time of the last check (Unix timestamp)'),
            'function': lambda item, req: int(item.last_chk),
            'datatype': int,
        },
        'last_hard_state': {
            'description': intern('The last hard state of the service'),
            'function': lambda item, req: item.last_hard_state,
            'datatype': int,
        },
        'last_hard_state_change': {
            'description': intern('The time of the last hard state change (Unix timestamp)'),
            'function': lambda item, req: item.last_hard_state_change,
            'datatype': int,
        },
        'last_notification': {
            'description': intern('The time of the last notification (Unix timestamp)'),
            'function': lambda item, req: int(item.last_notification),
            'datatype': int,
        },
        'last_state': {
            'description': intern('The last state of the service'),
            'function': lambda item, req: item.last_state,
            'datatype': int,
        },
        'last_state_change': {
            'description': intern('The time of the last state change (Unix timestamp)'),
            'function': lambda item, req: int(item.last_state_change),
            'datatype': int,
        },
        'last_time_critical': {
            'description': intern('The last time the service was CRITICAL (Unix timestamp)'),
            'function': lambda item, req: item.last_time_critical,  # CONTROLME INSORTME
            'datatype': int,
        },
        'last_time_warning': {
            'description': intern('The last time the service was in WARNING state (Unix timestamp)'),
            'function': lambda item, req: item.last_time_warning,  # CONTROLME INSORTME
            'datatype': int,
        },
        'last_time_ok': {
            'description': intern('The last time the service was OK (Unix timestamp)'),
            'function': lambda item, req: item.last_time_ok,  # CONTROLME INSORTME
            'datatype': int,
        },
        'last_time_unknown': {
            'description': intern('The last time the service was UNKNOWN (Unix timestamp)'),
        ...
        'long_plugin_output': {
            'description': intern('Unabbreviated output of the last check plugin'),
            'function': lambda item, req: item.long_output,
        },
        'low_flap_threshold': {
            'description': intern('Low threshold of flap detection'),
        ...
        'max_check_attempts': {
            'description': intern('The maximum number of check attempts'),
            'function': lambda item, req: item.max_check_attempts,
            'datatype': int,
        },
        'modified_attributes': {
        ...
        'next_check': {
            'description': intern('The scheduled time of the next check (Unix timestamp)'),
            'function': lambda item, req: int(item.next_chk),
            'datatype': int,
        },
        'next_notification': {
            'description': intern('The time of the next notification (Unix timestamp)'),
            'function': lambda item, req: int(item.last_notification + item.notification_interval * item.__class__.interval_length),  # CONTROLME
            'datatype': int,
        },
        'notes': {
            'description': intern('Optional notes about the service'),
            'function': lambda item, req: item.notes,
        },
        'notes_expanded': {
            'description': intern('The notes with (the most important) macros expanded'),
            'function': lambda item, req: MacroResolver().resolve_simple_macros_in_string(item.notes, item.get_data_for_checks()),
        },
        'notes_url': {
            'description': intern('An optional URL for additional notes about the service'),
            'function': lambda item, req: item.notes_url,
        },
        'notes_url_expanded': {
            'description': intern('The notes_url with (the most important) macros expanded'),
            'function': lambda item, req: MacroResolver().resolve_simple_macros_in_string(item.notes_url, item.get_data_for_checks()),
        },
        'notification_interval': {
            'description': intern('Interval of periodic notification or 0 if its off'),
        ...
        'notification_period': {
            'description': intern('The name of the notification period of the service. It this is empty, service problems are always notified.'),
            'function': lambda item, req: item.notification_period.get_name(),
        },
        'notifications_enabled': {
            'description': intern('Whether notifications are enabled for the service (0/1)'),
            'function': lambda item, req: item.notifications_enabled,
            'datatype': bool,
        },
        'no_more_notifications': {
        ...
        'obsess_over_service': {
            'description': intern('Whether \'obsess_over_service\' is enabled for the service (0/1)'),
            'function': lambda item, req: item.obsess_over_service,
            'datatype': bool,
        },
        'parent_dependencies': {
            'description': intern('List of the dependencies (logical, network or business one) of this service.'),
            'function': lambda item, req: from_svc_hst_distinct_lists(item.parent_dependencies),
            'datatype': list,
        },
        'percent_state_change': {
        ...
        'perf_data': {
            'description': intern('Performance data of the last check plugin'),
            'function': lambda item, req: item.perf_data,
        },
        'plugin_output': {
            'description': intern('Output of the last check plugin'),
            'function': lambda item, req: item.output,
        },
        'pnpgraph_present': {
            'description': intern('Whether there is a PNP4Nagios graph present for this service (0/1)'),
            'function': lambda item, req: find_pnp_perfdata_xml(item.get_full_name(), req),
            'datatype': int,
        },
        'process_performance_data': {
            'description': intern('Whether processing of performance data is enabled for the service (0/1)'),
            'function': lambda item, req: item.process_perf_data,
            'datatype': bool,
        },
        'retry_interval': {
        ...
        'scheduled_downtime_depth': {
            'description': intern('The number of scheduled downtimes the service is currently in'),
            'function': lambda item, req: item.scheduled_downtime_depth,
            'datatype': int,
        },
        'source_problems': {
        ...
        'state': {
            'description': intern('The current state of the service (0: OK, 1: WARN, 2: CRITICAL, 3: UNKNOWN)'),
            'function': lambda item, req: item.state_id,
            'datatype': int,
        },
        'state_type': {
            'description': intern('The type of the current state (0: soft, 1: hard)'),
            'function': lambda item, req: item.state_type_id,
            'datatype': int,
        },
    },
    'Hostgroup': {
        'action_url': {
            'description': intern('An optional URL to custom actions or information about the hostgroup'),
            'function': lambda item, req: item.action_url,
        },
        'alias': {
            'description': intern('An alias of the hostgroup'),
            'function': lambda item, req: item.alias,
        },
        'members': {
            'description': intern('A list of all host names that are members of the hostgroup'),
            'function': lambda item, req: [x.get_name() for x in item.members],
            'datatype': list,
        },
        'members_with_state': {
            'description': intern('A list of all host names that are members of the hostgroup together with state and has_been_checked'),
            'function': lambda item, req: [join_with_separators(req, x.get_name(), x.state_id, x.has_been_checked) for x in item.members],
            'datatype': list,
        },
        'name': {
            'description': intern('Name of the hostgroup'),
            'function': lambda item, req: item.hostgroup_name,
        },
        'notes': {
            'description': intern('Optional notes to the hostgroup'),
            'function': lambda item, req: item.notes,
        },
        'notes_url': {
            'description': intern('An optional URL with further information about the hostgroup'),
            'function': lambda item, req: item.notes_url,
        },
        'num_hosts': {
            'description': intern('The total number of hosts in the group'),
            'function': lambda item, req: len(item.members),
        },
        'num_hosts_down': {
            'description': intern('The number of hosts in the group that are down'),
            'function': lambda item, req: len([x for x in item.members if x.state_id == 1]),
        },
        'num_hosts_pending': {
            'description': intern('The number of hosts in the group that are pending'),
            'function': lambda item, req: len([x for x in item.members if x.has_been_checked == 0]),
        },
        'num_hosts_unreach': {
            'description': intern('The number of hosts in the group that are unreachable'),
            'function': lambda item, req: len([x for x in item.members if x.state_id == 2]),
        },
        'num_hosts_up': {
            'description': intern('The number of hosts in the group that are up'),
            'function': lambda item, req: len([x for x in item.members if x.state_id == 0]),
        },
        'num_services': {
            'description': intern('The total number of services of hosts in this group'),
            'function': lambda item, req: lambda x: sum((len(x.services) for x in item.members)),
        },
        'num_services_crit': {
            'description': intern('The total number of services with the state CRIT of hosts in this group'),
        ...
        'num_services_hard_ok': {
            'description': intern('The total number of services with the state OK of hosts in this group'),
            'function': lambda item, req: len([y for x in item.members for y in x.services if y.state_id == 0 and y.state_type_id == 1]),
        },
        'num_services_hard_unknown': {
            'description': intern('The total number of services with the state UNKNOWN of hosts in this group'),
            'function': lambda item, req: len([y for x in item.members for y in x.services if y.state_id == 3 and y.state_type_id == 1]),
        },
        'num_services_hard_warn': {
            'description': intern('The total number of services with the state WARN of hosts in this group'),
            'function': lambda item, req: len([y for x in item.members for y in x.services if y.state_id == 2 and y.state_type_id == 1]),
        },
        'num_services_ok': {
            'description': intern('The total number of services with the state OK of hosts in this group'),
        ...
        'num_services_pending': {
            'description': intern('The total number of services with the state Pending of hosts in this group'),
        ...
        'worst_host_state': {
            'description': intern('The worst state of all of the groups\' hosts (UP <= UNREACHABLE <= DOWN)'),
            'function': lambda item, req: reduce(worst_host_state, (x.state_id for x in item.members if x.state_type_id == 1), 0),
            'datatype': int,
        },
        'worst_service_hard_state': {
            'description': intern('The worst state of all services that belong to a host of this group (OK <= WARN <= UNKNOWN <= CRIT)'),
    ...
    'Servicegroup': {
        'action_url': {
            'description': intern('An optional URL to custom notes or actions on the service group'),
            'function': lambda item, req: item.action_url,
        },
        'alias': {
            'description': intern('An alias of the service group'),
            'function': lambda item, req: item.alias,
        },
        'members': {
            'description': intern('A list of all members of the service group as host/service pairs'),
            'function': lambda item, req: [get_livestatus_full_name(x, req) for x in item.members],
            'datatype': list,
        },
        'members_with_state': {
            'description': intern('A list of all members of the service group with state and has_been_checked'),
        ...
        'name': {
            'description': intern('The name of the service group'),
            'function': lambda item, req: item.servicegroup_name,
        },
        'notes': {
            'description': intern('Optional additional notes about the service group'),
            'function': lambda item, req: item.notes,
        },
        'notes_url': {
            'description': intern('An optional URL to further notes on the service group'),
            'function': lambda item, req: item.notes_url,
        },
        'num_services': {
            'description': intern('The total number of services in the group'),
            'function': lambda item, req: len(item.members),
            'datatype': int,
        },
        'num_services_crit': {
            'description': intern('The number of services in the group that are CRIT'),
            'function': lambda item, req: len([x for x in item.members if x.state_id == 2]),
            'datatype': int,
        },
        'num_services_hard_crit': {
        ...
        'num_services_hard_ok': {
            'description': intern('The number of services in the group that are OK'),
            'function': lambda item, req: len([x for x in item.members if x.state_id == 0 and x.state_type_id == 1]),
            'datatype': int,
        },
        'num_services_hard_unknown': {
            'description': intern('The number of services in the group that are UNKNOWN'),
            'function': lambda item, req: len([x for x in item.members if x.state_id == 3 and x.state_type_id == 1]),
            'datatype': int,
        },
        'num_services_hard_warn': {
            'description': intern('The number of services in the group that are WARN'),
        ...
        'num_services_pending': {
            'description': intern('The number of services in the group that are PENDING'),
            'function': lambda item, req: len([x for x in item.members if x.has_been_checked == 0]),
            'datatype': int,
        },
        'num_services_unknown': {
        ...
        'worst_service_state': {
            'description': intern('The worst soft state of all of the groups services (OK <= WARN <= UNKNOWN <= CRIT)'),
    ...
    'Contact': {
        'address1': {
            'description': intern('The additional field address1'),
            'function': lambda item, req: item.address1,
        },
        'address2': {
            'description': intern('The additional field address2'),
            'function': lambda item, req: item.address2,
        },
        'address3': {
            'description': intern('The additional field address3'),
            'function': lambda item, req: item.address3,
        },
        'address4': {
            'description': intern('The additional field address4'),
            'function': lambda item, req: item.address4,
        },
        'address5': {
            'description': intern('The additional field address5'),
            'function': lambda item, req: item.address5,
        },
        'address6': {
            'description': intern('The additional field address6'),
            'function': lambda item, req: item.address6,
        },
        'alias': {
            'description': intern('The full name of the contact'),
            'function': lambda item, req: item.alias,
        },
        'can_submit_commands': {
            'description': intern('Whether the contact is allowed to submit commands (0/1)'),
        ...
        'custom_variable_names': {
            'description': intern('A list of all custom variables of the contact'),
            'function': lambda item, req: get_customs_keys(item.customs),
            'datatype': list,
        },
        'custom_variable_values': {
            'description': intern('A list of the values of all custom variables of the contact'),
            'function': lambda item, req: get_customs_values(item.customs),
            'datatype': list,
        },
        'email': {
            'description': intern('The email address of the contact'),
            'function': lambda item, req: item.email,
        },
        'host_notification_period': {
            'description': intern('The time period in which the contact will be notified about host problems'),
            'function': lambda item, req: item.host_notification_period.get_name(),
        },
        'host_notifications_enabled': {
            'description': intern('Whether the contact will be notified about host problems in general (0/1)'),
            'function': lambda item, req: item.host_notifications_enabled,
            'datatype': bool,
        },
        'in_host_notification_period': {
            'description': intern('Whether the contact is currently in his/her host notification period (0/1)'),
        ...
        'in_service_notification_period': {
            'description': intern('Whether the contact is currently in his/her service notification period (0/1)'),
        ...
        'name': {
            'description': intern('The login name of the contact person'),
            'function': lambda item, req: item.contact_name,
        },
        'pager': {
            'description': intern('The pager address of the contact'),
            'function': lambda item, req: item.pager,
        },
        'service_notification_period': {
            'description': intern('The time period in which the contact will be notified about service problems'),
            'function': lambda item, req: item.service_notification_period.get_name(),
        },
        'service_notifications_enabled': {
            'description': intern('Whether the contact will be notified about service problems in general (0/1)'),
    ...
    'Contactgroup': {
        'alias': {
            'description': intern('The alias of the contactgroup'),
            'function': lambda item, req: item.alias,
        },
        'members': {
            'description': intern('A list of all members of this contactgroup'),
            'function': lambda item, req: [x.get_name() for x in item.members],
            'datatype': list,
        },
        'name': {
            'description': intern('The name of the contactgroup'),
            'function': lambda item, req: item.contactgroup_name,
        },
    },
    'Timeperiod': {
        'alias': {
            'description': intern('The alias of the timeperiod'),
            'function': lambda item, req: item.alias,
        },
        'in': {
            'description': intern('Wether we are currently in this period (0/1)'),
            'function': lambda item, req: item.is_in,  # CONTROLME REPAIRME
            'datatype': int,
        },
        'name': {
            'description': intern('The name of the timeperiod'),
            'function': lambda item, req: item.timeperiod_name,
        },
    },
    'Command': {
        'line': {
            'description': intern('The shell command line'),
            'function': lambda item, req: item.command_line,
        },
        'name': {
            'description': intern('The name of the command'),
            'function': lambda item, req: item.command_name,
        },
    },
    'SchedulerLink': {
        'address': {
            'description': intern('The ip or dns adress ofthe scheduler'),
            'function': lambda item, req: item.address,  # REPAIRME
        },
        'alive': {
            'description': intern('If the scheduler is alive or not'),
            'function': lambda item, req: item.alive,
            'datatype': bool,
        },
        'name': {
            'description': intern('The name of the scheduler'),
            'function': lambda item, req: item.scheduler_name,  # REPAIRME
        },
        'port': {
            'description': intern('The TCP port of the scheduler'),
            'function': lambda item, req: item.port,  # REPAIRME
            'datatype': int,
        },
        'spare': {
            'description': intern('If the scheduler is a spare or not'),
            'function': lambda item, req: item.spare,
            'datatype': bool,
        },
        'weight': {
            'description': intern('Weight (in terms of hosts) of the scheduler'),
    ...
    'PollerLink': {
        'address': {
            'description': intern('The ip or dns adress of the poller'),
            'function': lambda item, req: item.address,  # REPAIRME
        },
        'alive': {
            'description': intern('If the poller is alive or not'),
            'function': lambda item, req: item.alive,
            'datatype': bool,
        },
        'name': {
            'description': intern('The name of the poller'),
            'function': lambda item, req: item.poller_name,  # REPAIRME
        },
        'port': {
            'description': intern('The TCP port of the poller'),
            'function': lambda item, req: item.port,  # REPAIRME
            'datatype': int,
        },
        'spare': {
            'description': intern('If the poller is a spare or not'),
            'function': lambda item, req: item.spare,
            'datatype': bool,
        },
    },
    'ReactionnerLink': {
        'address': {
            'description': intern('The ip or dns adress of the reactionner'),
            'function': lambda item, req: item.address,  # REPAIRME
        },
        'alive': {
            'description': intern('If the reactionner is alive or not'),
            'function': lambda item, req: item.alive,
            'datatype': bool,
        },
        'name': {
            'description': intern('The name of the reactionner'),
            'function': lambda item, req: item.reactionner_name,  # REPAIRME
        },
        'port': {
            'description': intern('The TCP port of the reactionner'),
            'function': lambda item, req: item.port,  # REPAIRME
            'datatype': int,
        },
        'spare': {
            'description': intern('If the reactionner is a spare or not'),
            'function': lambda item, req: item.spare,
            'datatype': bool,
        },
    },
    'BrokerLink': {
        'address': {
            'description': intern('The ip or dns adress of the broker'),
            'function': lambda item, req: item.address,  # REPAIRME
        },
        'alive': {
            'description': intern('If the broker is alive or not'),
            'function': lambda item, req: item.alive,
            'datatype': bool,
        },
        'name': {
            'description': intern('The name of the broker'),
            'function': lambda item, req: item.broker_name,  # REPAIRME
        },
        'port': {
            'description': intern('The TCP port of the broker'),
            'function': lambda item, req: item.port,  # REPAIRME
            'datatype': int,
        },
        'spare': {
            'description': intern('If the broker is a spare or not'),
            'function': lambda item, req: item.spare,
            'datatype': bool,
        },
    },
    'Problem': {
        ...
        'source': {
            'description': intern('The source name of the problem (host or service)'),
            'function': lambda item, req: get_livestatus_full_name(item.source, req),
        },
    },
    'Downtime': {
        'author': {
            'description': intern('The contact that scheduled the downtime'),
            'function': lambda item, req: item.author,
        },
        'comment': {
            'description': intern('A comment text'),
            'function': lambda item, req: item.comment,
        },
        'duration': {
            'description': intern('The duration of the downtime in seconds'),
            'function': lambda item, req: item.duration,
            'datatype': int,
        },
        'end_time': {
            'description': intern('The end time of the downtime as UNIX timestamp'),
            'function': lambda item, req: item.end_time,
            'datatype': int,
        },
        'entry_time': {
            'description': intern('The time the entry was made as UNIX timestamp'),
            'function': lambda item, req: item.entry_time,
            'datatype': int,
        },
        'fixed': {
            'description': intern('A 1 if the downtime is fixed, a 0 if it is flexible'),
        ...
        'host_comments_with_info': {
            'description': intern('A list of all comments of the host with id, author and comment'),
        },
        'host_contacts': {
            'description': intern('A list of all contacts of this host, either direct or via a contact group'),
        },
        ...
        'id': {
            'description': intern('The id of the downtime'),
            'function': lambda item, req: item.id,
            'datatype': int,
        },
        'is_service': {
            'description': intern('0, if this entry is for a host, 1 if it is for a service'),
        ...
        'service_comments_with_info': {
            'description': intern('A list of all comments of the service with id, author and comment'),
        },
        'service_contacts': {
            'description': intern('A list of all contacts of the service, either direct or via a contact group'),
        },
        ...
        'start_time': {
            'description': intern('The start time of the downtime as UNIX timestamp'),
            'function': lambda item, req: item.start_time,
            'datatype': int,
        },
        'triggered_by': {
            'description': intern('The id of the downtime this downtime was triggered by or 0 if it was not triggered by another downtime'),
            'function': lambda item, req: item.trigger_id,
            'datatype': int,
        },
        'type': {
            'description': intern('The type of the downtime: 0 if it is active, 1 if it is pending'),
    ...
    'Comment': {
        'author': {
            'description': intern('The contact that entered the comment'),
            'function': lambda item, req: item.author,
        },
        'comment': {
            'description': intern('A comment text'),
        ...
        'entry_type': {
            'description': intern('The type of the comment: 1 is user, 2 is downtime, 3 is flap and 4 is acknowledgement'),
            'function': lambda item, req: item.entry_type,
            'datatype': int,
        },
        'expire_time': {
            'description': intern('The time of expiry of this comment as a UNIX timestamp'),
            'function': lambda item, req: item.expire_time,
            'datatype': int,
        },
        'expires': {
            'description': intern('Whether this comment expires'),
        ...
        'id': {
            'description': intern('The id of the comment'),
            'function': lambda item, req: item.id,
            'datatype': int,
        },
        'is_service': {
        ...
        'persistent': {
            'description': intern('Whether this comment is persistent (0/1)'),
        ...
        'source': {
            'description': intern('The source of the comment (0 is internal and 1 is external)'),
            'function': lambda item, req: item.source,
            'datatype': int,
        },
        'type': {
            'description': intern('The type of the comment: 1 is host, 2 is service'),
    ...
    'Config': {
        'accept_passive_host_checks': {
            'description': intern('Whether passive host checks are accepted in general (0/1)'),
            'function': lambda item, req: item.passive_host_checks_enabled,
            'datatype': bool,
        },
        'accept_passive_service_checks': {
            'description': intern('Whether passive service checks are activated in general (0/1)'),
            'function': lambda item, req: item.passive_service_checks_enabled,
            'datatype': bool,
        },
        'cached_log_messages': {
            'description': intern('The current number of log messages MK Livestatus keeps in memory'),
        ...
        'check_external_commands': {
            'description': intern('Whether Nagios checks for external commands at its command pipe (0/1)'),
            'function': lambda item, req: item.check_external_commands,
            'datatype': bool,
        },
        'check_host_freshness': {
            'description': intern('Whether host freshness checking is activated in general (0/1)'),
            'function': lambda item, req: item.check_host_freshness,
            'datatype': bool,
        },
        'check_service_freshness': {
            'description': intern('Whether service freshness checking is activated in general (0/1)'),
            'function': lambda item, req: item.check_service_freshness,
            'datatype': bool,
        },
        'connections': {
            'description': intern('The number of client connections to Livestatus since program start'),
            'function': lambda item, req: 0,  # REPAIRME
            'datatype': int,
        },
        'connections_rate': {
            'description': intern('The averaged number of new client connections to Livestatus per second'),
            'function': lambda item, req: 0,  # REPAIRME
            'datatype': float,
        },
        'enable_event_handlers': {
            'description': intern('Whether event handlers are activated in general (0/1)'),
            'function': lambda item, req: item.event_handlers_enabled,
            'datatype': bool,
        },
        'enable_flap_detection': {
            'description': intern('Whether flap detection is activated in general (0/1)'),
            'function': lambda item, req: item.flap_detection_enabled,
            'datatype': bool,
        },
        'enable_notifications': {
            'description': intern('Whether notifications are enabled in general (0/1)'),
            'function': lambda item, req: item.notifications_enabled,
            'datatype': bool,
        },
        'execute_host_checks': {
            'description': intern('Whether host checks are executed in general (0/1)'),
            'function': lambda item, req: item.active_host_checks_enabled,
            'datatype': bool,
        },
        'execute_service_checks': {
            'description': intern('Whether active service checks are activated in general (0/1)'),
            'function': lambda item, req: item.active_service_checks_enabled,
            'datatype': bool,
        },
        'external_command_buffer_max': {
            'description': intern('The maximum number of slots used in the external command buffer'),
            'function': lambda item, req: item.external_command_buffer_max,  # REPAIRME
            'datatype': int,
        },
        'external_command_buffer_slots': {
            'description': intern('The size of the buffer for the external commands'),
            'function': lambda item, req: item.external_command_buffer_slots,  # REPAIRME
            'datatype': int,
        },
        'external_command_buffer_usage': {
            'description': intern('The number of slots in use of the external command buffer'),
            'function': lambda item, req: item.external_command_buffer_usage,  # REPAIRME
            'datatype': int,
        },
        'external_commands_rate': {
            'description': intern('The averaged number of external commands per second'),
            'function': lambda item, req: 0,  # REPAIRME
            'datatype': float,
        },
        'forks': {
            'description': intern('The number of process creations since program start'),
            'function': lambda item, req: 0,  # REPAIRME
            'datatype': float,
        },
        'forks_rate': {
            'description': intern('The averaged number of forks per second'),
            'function': lambda item, req: 0,  # REPAIRME
            'datatype': float,
        },
        'host_checks': {
            'description': intern('The number of host checks since program start'),
            'function': lambda item, req: 0,  # REPAIRME
            'datatype': int,
        },
        'host_checks_rate': {
            'description': intern('the averaged number of host checks per second'),
            'function': lambda item, req: 0,  # REPAIRME
            'datatype': float,
        },
        'interval_length': {
            'description': intern('The default interval length from nagios.cfg'),
            'function': lambda item, req: item.interval_length,  # REPAIRME
            'datatype': int,
        },
        'last_command_check': {
            'description': intern('The time of the last check for a command as UNIX timestamp'),
            'function': lambda item, req: item.last_command_check,  # REPAIRME
            'datatype': int,
        },
        'last_log_rotation': {
            'description': intern('Time time of the last log file rotation'),
            'function': lambda item, req: item.last_log_rotation,  # REPAIRME
            'datatype': int,
        },
        'livestatus_version': {
            'description': intern('The version of the MK Livestatus module'),
            'function': lambda item, req: '2.0-shinken',
        },
        'log_messages': {
            'description': intern('The number of new log messages since program start'),
            'function': lambda item, req: 0,  # REPAIRME
            'datatype': float,
        },
        'log_messages_rate': {
            'description': intern('The averaged number of log messages per second'),
            'function': lambda item, req: 0,  # REPAIRME
            'datatype': float,
        },
        'nagios_pid': {
            'description': intern('The process ID of the Nagios main process'),
            'function': lambda item, req: item.pid,  # REPAIRME
            'datatype': int,
        },
        'neb_callbacks': {
            'description': intern('The number of NEB call backs since program start'),
            'function': lambda item, req: 0,  # REPAIRME
            'datatype': int,
        },
        'neb_callbacks_rate': {
            'description': intern('The averaged number of NEB call backs per second'),
            'function': lambda item, req: 0,  # REPAIRME
            'datatype': float,
        },
        'obsess_over_hosts': {
            'description': intern('Whether Nagios will obsess over host checks (0/1)'),
            'function': lambda item, req: item.obsess_over_hosts,
            'datatype': bool,
        },
        'obsess_over_services': {
            'description': intern('Whether Nagios will obsess over service checks and run the ocsp_command (0/1)'),
            'function': lambda item, req: item.obsess_over_services,
            'datatype': bool,
        },
        'process_performance_data': {
            'description': intern('Whether processing of performance data is activated in general (0/1)'),
            'function': lambda item, req: item.process_performance_data,
            'datatype': bool,
        },
        'program_start': {
            'description': intern('The time of the last program start as UNIX timestamp'),
            'function': lambda item, req: 0,  # REPAIRME
            'datatype': int,
        },
        'program_version': {
            'description': intern('The version of the monitoring daemon'),
            'function': lambda item, req: VERSION,
        },
        'requests': {
            'description': intern('The number of requests to Livestatus since program start'),
            'function': lambda item, req: 0,  # REPAIRME
            'datatype': int,
        },
        'requests_rate': {
            'description': intern('The averaged number of request to Livestatus per second'),
            'function': lambda item, req: 0,  # REPAIRME
            'datatype': float,
        },
        'service_checks': {
            'description': intern('The number of completed service checks since program start'),
            'function': lambda item, req: 0,  # REPAIRME
            'datatype': int,
        },
        'service_checks_rate': {
            'description': intern('The averaged number of service checks per second'),
            'function': lambda item, req: 0,  # REPAIRME
            'datatype': float,
        },
    },
    'Logline': {
        'attempt': {
            'description': intern('The number of the check attempt'),
            'function': lambda item, req: item.attempt,
            'datatype': int,
        },
        'class': {
            'description': intern('The class of the message as integer (0:info, 1:state, 2:program, 3:notification, 4:passive, 5:command)'),
            'function': lambda item, req: item.logclass,
            'datatype': int,
        },
        'command_name': {
            'description': intern('The name of the command of the log entry (e.g. for notifications)'),
            'function': lambda item, req: item.command_name,
        },
        'comment': {
            'description': intern('A comment field used in various message types'),
            'function': lambda item, req: item.comment,
        },
        'contact_name': {
            'description': intern('The name of the contact the log entry is about (might be empty)'),
            'function': lambda item, req: item.contact_name,
        },
        ...
        'host_name': {
            'description': intern('The name of the host the log entry is about (might be empty)'),
            'function': lambda item, req: item.host_name,
        },
        'lineno': {
            'description': intern('The number of the line in the log file'),
            'function': lambda item, req: item.lineno,
            'datatype': int,
        },
        'message': {
            'description': intern('The complete message line including the timestamp'),
            'function': lambda item, req: item.message,
        },
        'options': {
            'description': intern('The part of the message after the \':\''),
            'function': lambda item, req: item.options,
        },
        'plugin_output': {
            'description': intern('The output of the check, if any is associated with the message'),
            'function': lambda item, req: item.plugin_output,
        },
        'service_description': {
            'description': intern('The description of the service log entry is about (might be empty)'),
            'function': lambda item, req: item.service_description,
        },
        'state': {
            'description': intern('The state of the host or service in question'),
            'function': lambda item, req: item.state,
            'datatype': int,
        },
        'state_type': {
            'description': intern('The type of the state (varies on different log classes)'),
            'function': lambda item, req: item.state_type,  # REPAIRME
        },
        'time': {
            'description': intern('Time of the log event (UNIX timestamp)'),
            'function': lambda item, req: item.time,  # REPAIRME
            'datatype': int,
        },
        'type': {
            'description': intern('The type of the message (text before the colon), the message itself for info messages'),
